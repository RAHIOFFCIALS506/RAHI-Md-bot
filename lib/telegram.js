...
export default async function initializeTelegramBot(manager) {
  ...
  async function handleCommand(msg) {
    ...
    try {
      ...
      await safeReply(chatId,
        ...
        { reply_to_message_id: replyId }).catch(() => {});
    }
  }
  ...
  tbot.on("callback_query", async (query) => {
    ...
    try {
      ...
      if (data.startsWith("repairr:")) {
        ...
        if (remain > 0) {
          return tbot.answerCallbackQuery(id, {
            text: `⏳ ᴡᴀɪᴛ ${Math.ceil(remain / 1000)}ꜱ`,
            show_alert: true,
          }).catch(() => {});
        }
        pairCooldown.set(userId, Date.now());
    ...
    } catch (e) { LOG.error("ᴄᴀʟʟʙᴀᴄᴋ ᴇʀʀᴏʀ:", e); }
  });
  ...
  tbot.on("message", async (msg) => {
    try {
      const sender = msg.from?.username || msg.from?.id || msg.sender_chat?.id || "?";
      LOG.msg(msg.chat?.id, msg.chat?.type, sender, msg.text || "");
    } catch {}

    try { await handleCommand(msg); }
    catch (e) { LOG.error("ʜᴀɴᴅʟᴇ ᴇʀʀᴏʀ:", e); }
  });
  ...
  if (USE_WEBHOOK) {
    try {
      ...
      LOG.ok("ᴡᴇʙʜᴏᴏᴋ ꜱᴇᴛ:", webhookUrl);
      const info = await tbot.getWebHookInfo();
      LOG.info("ᴡᴇʙʜᴏᴏᴋ:", info.url, info.last_error_message || "ᴏᴋ");
    } catch (e) {
      LOG.error("ᴡᴇʙʜᴏᴏᴋ ꜰᴀɪʟᴇᴅ:", e?.message);
    }
  } else {
    LOG.info("ᴘᴏʟʟɪɴɢ ᴍᴏᴅᴇ.");
  }
